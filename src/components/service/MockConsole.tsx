...
export default function MockConsole({ serviceType, serviceName }: MockConsoleProps) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-colors ${
      ...
      {/* Console Body */}
      <div className="p-4">
        {serviceType === 'storage' && <StorageView isExamLens={isExamLens} />}
        {serviceType === 'training' && <TrainingView isExamLens={isExamLens} />}
        {serviceType === 'registry' && <RegistryView isExamLens={isExamLens} />}
        {serviceType === 'orchestration' && <OrchestrationView isExamLens={isExamLens} />}
      </div>
    </div>
  );
}

function StorageView({ isExamLens }: { isExamLens: boolean }) {
  const files = [
    { name: 'training-data/', type: 'folder', size: '—' },
    { name: 'model-artifacts/', type: 'folder', size: '—' },
    { name: 'raw-dataset.csv', type: 'file', size: '2.4 GB' },
    { name: 'features.parquet', type: 'file', size: '890 MB' },
  ];
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        ...
        {files.map((file, i) => (
          <div 
            key={i}
            className="grid grid-cols-3 gap-4 px-3 py-2.5 border-b border-border last:border-0 hover:bg-muted/20 cursor-pointer transition-colors"
          >
...
function TrainingView({ isExamLens }: { isExamLens: boolean }) {
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        ...
        {jobs.map((job, i) => (
          <div 
            key={i}
            className="grid grid-cols-4 gap-2 px-3 py-2.5 border-b border-border last:border-0 hover:bg-muted/20 cursor-pointer transition-colors"
          >
...
function RegistryView({ isExamLens }: { isExamLens: boolean }) {
  const models = [
  ...
  const statusBadge = (status: string) => {
    if (status === 'Approved') {
      return <span className="text-[10px] px-2 py-0.5 rounded bg-green-500/10 text-green-400">Approved</span>;
    }
    return <span className="text-[10px] px-2 py-0.5 rounded bg-yellow-500/10 text-yellow-400">Pending</span>;
  };
...
function OrchestrationView({ isExamLens }: { isExamLens: boolean }) {
  const steps = [
...
function ExamHighlight({ children }: { children: React.ReactNode }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 5 }}
      animate={{ opacity: 1, y: 0 }}
      className="p-3 rounded-lg border border-primary/30 bg-primary/5 text-xs text-foreground/80 leading-relaxed"
    >
      {children}
    </motion.div>
  );
